...
const query = `
query CheckAvailability($carModel: String!, $postcode: String!) {  getCarAvailability(carModel: $carModel, postcode: $postcode) {   date
  colours
	  __typename	  }
}
`;
...
const getModels = async function () {
  ...
  for (const modelSlug of Object.keys(models)) {
	...
	if (model.availability.length) {
		model.availability.forEach(function(availability) {
			const date = new Date();
			const sniffTimestamp = date.getFullYear() + ("0" + (date.getMonth() + 1)).slice(-2) + ("0" + date.getDate()).slice(-2) + ("0" + date.getHours() ).slice(-2) + ("0" + date.getMinutes()).slice(-2) + ("0" + date.getSeconds()).slice(-2);
  ...
  console.log(`All done :), check ${csvPath} for results. Found: ${availableCarDates.length} car/dates`);
};
...
const getModel = async function (modelSlug) {
  ...
  models[modelSlug].jsonLink = `https://on.to/page-data/electric-cars/${modelSlug}/page-data.json`;
